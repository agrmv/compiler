...
public class MIPSGenVisitor implements IRVisitor {
	...
	ArrayList<AssemblyHelper> assemblyHelp = new ArrayList<>();
    HashMap<String, Integer> dataSection = new HashMap<>();
	...
	public void visit(call i) {
        //  Если мы находимся в функции, сохраняем наши аргументы перед загрузкой новых
        ...
        // Разверните стек для хранения аргументов
        emit(new AssemblyHelper("sub", "$sp", "$sp", "" + (i.args.size() * 4)));

        // Теперь загрузите аргументы
        int arg = 0;
	...
	public void visit(callr i) {
        ...
        // Разварачиваем стек для хранения аргументов
        emit(new AssemblyHelper("sub", "$sp", "$sp", "" + (i.args.size() * 4)));

        // Теперь загружаем аргументы
        int arg = 0;
        for (Operand o : i.args) {
            if (arg < 3) {
                if (o instanceof Register) {
        ...
        // Помещаем возвращаемое значение в регистр назначения
        ...
        // Восстанавливаем наши аргументы {
        if (currentFunction != null) {
            if (currentFunction.argumentCount > 0 && i.args.size() > 0) {
                int baseOffset = currentFunction.usedRegsCount * 4 + 12;
                emit(new AssemblyHelper("lw", "$a0", "" + baseOffset + "($fp)", ""));
	...
	public void visit(SharedLabel i) {
		 emit(new AssemblyHelper(i.name + ":", "", "",""));
	}
    ...
    public void visit(FunctionEpilogue i) {
        ...
        // Восстановить сохраненные временные регистры
        ...
        // Указатель поп-кадра и адрес возврата
    ...
    public void visit(FunctionPrologue i) {
        ...
        // Сохраняем все регистры gp, которые мы используем
        ...
        // Устанавиваем указатель
        emit(new AssemblyHelper("move", "$fp", "$sp", ""));

        //Выделите место для всех наших временных переменных
